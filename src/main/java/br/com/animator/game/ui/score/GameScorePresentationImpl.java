...
public class GameScorePresentationImpl implements GameScorePresentation {
	...
	// --- Properties
	private double hiScoreCounter = 0;
	...
	private double[] individualRankPosition = new double[10];
	private Integer maxNickToShow = 0;
	private double[] individualNickPosition = new double[30];
	private Integer maxScoreToShow = 0;
	private double[] individualScorePosition = new double[30];
	...
	public GameScorePresentationImpl(Integer pwidth, Integer pheight, Integer currentAspectRatio) {
		...
		for (int cnt = 0; cnt < 10; cnt++) {
			...
			copy.createGraphics().drawImage(numbersImages, 0, 0, NUMBER_W, NUMBER_Y, cnt * NUMBER_W, 0,
					(cnt * NUMBER_W) + NUMBER_W, NUMBER_Y, null);
			this.numbers[cnt] = copy;
		}

		copy = graphicsConfig.createCompatibleImage(2 * NUMBER_W, NUMBER_Y, transparency);
		copy.createGraphics().drawImage(numbersImages, 0, 0, NUMBER_W, NUMBER_Y, NUMBER_W, 0, (2 * NUMBER_W), NUMBER_Y,
				null);
		copy.createGraphics().drawImage(numbersImages, NUMBER_W, 0, 2 * NUMBER_W, NUMBER_Y, 0, 0, NUMBER_W, NUMBER_Y,
				null);
		...
		for (int cnt = 0; cnt < 26; cnt++) {
			...
			copy.createGraphics().drawImage(lowersImages, 0, 0, LOWER_W, LOWER_Y, cnt * LOWER_W, 0,
					(cnt * LOWER_W) + LOWER_W, LOWER_Y, null);
		...
		for (int cnt = 0; cnt < 26; cnt++) {
			...
			copy.createGraphics().drawImage(uppersImages, 0, 0, UPPER_W, UPPER_Y, cnt * UPPER_W, 0,
					(cnt * UPPER_W) + UPPER_W, UPPER_Y, null);
			this.upperLetter[cnt] = copy;
		}

		for (int cnt = String.valueOf("A").codePointAt(0); cnt <= String.valueOf("Z").codePointAt(0); cnt++) {
			this.mapLetters.put(String.valueOf(Character.toChars(cnt)),
					this.upperLetter[cnt - String.valueOf("A").codePointAt(0)]);
			this.mapLetters.put(String.valueOf(Character.toChars(cnt + 32)),
					this.lowerLetter[cnt + 32 - String.valueOf("a").codePointAt(0)]);
		}
	...
	public void update(long frametime) {
		this.hiScoreCounter = hiScoreCounter + (1d * GameGlobals.GAME_VELOCITY);
	}
	...
	public void draw(Graphics2D g2d) {
		...
		int finalRankPosition = (this.PWIDTH / 2) - this.imageUtil.getScaledWidth(NAME_SCORE_SPACE) / 2
				- (5 * this.imageUtil.getScaledWidth(UPPER_W));
				...
				for (String key : this.gameScore.getSortedScoresSet()) {
					...
					if (image != null) {
						...
						if (this.individualRankPosition[rankCounter] < finalRankPosition) {
							this.individualRankPosition[rankCounter] = individualRankPosition[rankCounter] + (20d * GameGlobals.GAME_VELOCITY);
						}
						if (this.individualRankPosition[rankCounter] > finalRankPosition) {
						...
						g2d.drawImage(image,
								(int)this.individualRankPosition[rankCounter],
								height,
						...
						if (this.individualRankPosition[rankCounter] > finalRankPosition / 4) {
							...
							for (int cnt = 0; cnt < key.length(); cnt++) {
								...
								if (this.individualNickPosition[nickCounter] > finalNickPosition) {
									this.individualNickPosition[nickCounter] = individualNickPosition[nickCounter] - (30d * GameGlobals.GAME_VELOCITY);
								}
								if (this.individualNickPosition[nickCounter] < finalNickPosition) {
								...
								if (this.individualNickPosition[nickCounter] != finalNickPosition) {
									image = this.imageUtil.getRotatedImage(image,
											(int)this.individualNickPosition[nickCounter] % 360);
								}
								if (image != null) {
									g2d.drawImage(image,
											(int)this.individualNickPosition[nickCounter],
											height,
											this.imageUtil.getScaledWidth(image.getWidth()),
											this.imageUtil.getScaledHeight(image.getHeight()),
											null);
							...
							if (this.nickDone) {
								...
								for (int cnt = 0; cnt < value.length(); cnt++) {
									...
									int finalScorePosition = actualScorePosition
											+ this.imageUtil.getScaledWidth(NAME_SCORE_SPACE);
									int finalScorePositionY = height;

									if (this.individualScorePosition[scoreCounter] > finalScorePositionY) {
										this.individualScorePosition[scoreCounter] = individualScorePosition[scoreCounter]
												- (60d * GameGlobals.GAME_VELOCITY);
									}
									if (this.individualScorePosition[scoreCounter] < finalScorePositionY) {
									...
									if (image != null) {
										g2d.drawImage(image,
												finalScorePosition,
												(int)this.individualScorePosition[scoreCounter],
												this.imageUtil.getScaledWidth(image.getWidth()),
												this.imageUtil.getScaledHeight(image.getHeight()),
												null);
									}
						...
						// ---------------------------------------------------------//
						if (this.individualRankPosition[rankCounter] == finalRankPosition && this.nickDone
								&& this.scoreDone) {
