...
public class JoystickDetector {
    ...
    public static class ControllerInfo {
        ...
        public boolean isGamepad() { return isGamepad; }
    ...
    /**
     * Scans all ports and returns a list of information about the connected controllers.
    ...
    /**
     * Updates the state of all connected joysticks and detects changes (Buttons, Axes, and Hats).
     */
    public void update() {
        GLFW.glfwPollEvents();
    ...
    private void processButtons(ControllerInfo controller) {
        ...
        for (int b = 0; b < buttons.limit() && b < 64; b++) {
            ...
            if (currentlyPressed && !buttonHistory[jid][b]) {
                System.out.printf("[%s] Type: %s | Button Pressed: %d%n", 
                    controller.getName(), 
                    controller.isGamepad() ? "Mapped Gamepad" : "Generic Joystick", 
                    b);
            }
    ...
    private void processHats(ControllerInfo controller) {
        ...
        for (int h = 0; h < hats.limit() && h < 64; h++) {
            ...
            if (currentHat != hatHistory[jid][h]) {
                System.out.printf("[%s] DPAD/Hat Alterado: %d | Estado: %d%n", controller.getName(), h, currentHat);
    ...
    /**
     * Método main para testes rápidos de detecção e mapeamento.
     * Execute esta classe para identificar os IDs dos botões do seu 8BitDo.
     */
    public static void main(String[] args) {
        JoystickDetector detector = new JoystickDetector();
        if (!detector.initialize()) return;

        // Validação inicial: encerra o teste se nenhum controle estiver plugado
        if (!detector.checkJoystickAvailability()) {
            return;
        }

        System.out.println("=== Monitor de Joystick Iniciado ===");
        System.out.println("Aguardando entrada de dados... (Pressione Ctrl+C para sair)");

        while (true) {
            detector.update();

            try {
                // Pausa curta para evitar consumo excessivo de CPU no teste
                Thread.sleep(10);
